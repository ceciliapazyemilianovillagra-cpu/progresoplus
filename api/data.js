...
export default async function handler(req,res){
 try{
  ...
  if(p.action==='session'){const settings=row(await sql`SELECT recordatorios_activos,canal_recordatorio,telegram_chat_id,zona_horaria FROM configuracion_usuario WHERE usuario_id=${user.id}`);return res.status(200).json({ok:true,data:{user:account,settings}})}
  if(['listUsers','createUser','updateUser','resetPassword','deleteUser','getAiMonitoring','testTelegram'].includes(p.action)&&account.rol!=='admin')throw Error('Solo el administrador puede gestionar usuarios');
  let data;
  switch(p.action){
   case 'getAiMonitoring':data=await getAiMonitoring();break;
   case 'listBooks':{let files=[];try{files=readdirSync(BOOKS_DIR).filter(f=>f.endsWith('.json'))}catch{files=[]}data=files.map(f=>{try{const libro=JSON.parse(readFileSync(join(BOOKS_DIR,f),'utf8'));return{archivo:f,titulo:libro.titulo||f,autor:libro.autor||'',capitulos:Array.isArray(libro.capitulos)?libro.capitulos.length:0}}catch{return null}}).filter(Boolean);break}
   case 'getMotivacion':{let files=[];try{files=readdirSync(BOOKS_DIR).filter(f=>f.endsWith('.json'))}catch{throw Error('No hay libros cargados todavía')}if(!files.length)throw Error('No hay libros cargados todavía');if(p.archivo)files=files.filter(f=>f===p.archivo);if(!files.length)throw Error('Libro no encontrado');const candidatos=[];for(const f of files){try{const libro=JSON.parse(readFileSync(join(BOOKS_DIR,f),'utf8'));for(const c of(Array.isArray(libro.capitulos)?libro.capitulos:[]))candidatos.push({archivo:f,libro:libro.titulo||f,autor:libro.autor||'',capitulo:c})}catch{}}if(!candidatos.length)throw Error('No hay capítulos cargados todavía');const excluir=Array.isArray(p.excluir)?p.excluir:[];let disponibles=candidatos.filter(c=>!excluir.includes(c.capitulo.titulo));if(!disponibles.length)disponibles=candidatos;data=disponibles[Math.floor(Math.random()*disponibles.length)];break}
   ...
   case 'saveSettings':data=row(await sql`UPDATE configuracion_usuario SET recordatorios_activos=${p.recordatorios_activos===true||p.recordatorios_activos==='true'},canal_recordatorio=${['ninguno','telegram'].includes(p.canal_recordatorio)?p.canal_recordatorio:'ninguno'},zona_horaria=${String(p.zona_horaria||'America/Argentina/Buenos_Aires')},actualizado=now() WHERE usuario_id=${user.id} RETURNING recordatorios_activos,canal_recordatorio,telegram_chat_id,zona_horaria`);break;
   case 'linkTelegram':{if(!process.env.SUPABASE_FUNCTIONS_URL||!process.env.WA_CRON_SECRET)throw Error('Falta configurar la integración de Telegram en Vercel');const r=await fetch(`${process.env.SUPABASE_FUNCTIONS_URL}/tareas-whatsapp`,{method:'POST',headers:{'Content-Type':'application/json','x-cron-secret':process.env.WA_CRON_SECRET},body:JSON.stringify({action:'link',usuario_id:user.id})});const j=await r.json();if(!r.ok)throw Error(j.error||'No se pudo vincular Telegram');if(!j.linked)throw Error('Todavía no encontramos tu mensaje. Abrí el bot en Telegram, tocá Iniciar y volvé a intentar.');data={linked:true};break}
   case 'testTelegram':{const settings=row(await sql`SELECT telegram_chat_id FROM configuracion_usuario WHERE usuario_id=${user.id}`);if(!settings?.telegram_chat_id)throw Error('Primero vinculá tu Telegram');if(!process.env.SUPABASE_FUNCTIONS_URL||!process.env.WA_CRON_SECRET)throw Error('Falta configurar la integración de Telegram en Vercel');const r=await fetch(`${process.env.SUPABASE_FUNCTIONS_URL}/tareas-whatsapp`,{method:'POST',headers:{'Content-Type':'application/json','x-cron-secret':process.env.WA_CRON_SECRET},body:JSON.stringify({action:'test',chat_id:settings.telegram_chat_id})});const j=await r.json();if(!r.ok||j.error)throw Error(j.error||'No se pudo enviar el mensaje de prueba');data={sent:true,result:j};break}
